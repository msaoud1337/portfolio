...
export default function ResumeArticle() {
  const [icons, setIcons] = useState<{ href: string; title: string }[]>([]);
  const [isOpen, setIsOpen] = useState<boolean>(false);
  useEffect(() => {
    const addIconWithDelay = async () => {
      for (const icon of array) {
        await new Promise((resolve) => setTimeout(resolve, 600));
        const updatedIconState = !icons.includes(icon) ? [...icons, icon] : [...icons];
        setIcons(updatedIconState);
      }
    };
            ...
            <Stack direction={'row'} flexWrap={'wrap'} gap={1} pb={2}>
              {icons.map((icon, id) => (
                <Tooltip key={id} title={icon.title}>
                  <motion.img height={40} width={40} {...varFadeInRight} src={icon.href} />
                </Tooltip>
              ))}
            </Stack>
          </Box>
